    ...
    <div className="flex justify-center items-center w-full h-fit gap-4">
      <div
        className={`${
          ...
          !svgNo ? "opacity-100" : "opacity-0"
        } shadow-2xl h-full flex justify-center items-center relative w-[28%] lg:w-[15%] min-h-[220px] md:min-h-[260px] lg:min-h-[272px] rounded-3xl my-[4vh] p-4 m-2 py-auto md:p-3 overflow-hidden`}
      >
        <img
          src="/grid2.png"
          alt="Grid"
      ...
      <div className="py-[10px] pr-[20%] w-[65%] lg:max-w-[80%] relative min-h-[210px] md:min-h-[250px] lg:min-h-[262px] overflow-hidden bg-blue-200 p-4 serviceBox">
        <div className="flex justify-start items-center w-full gap-2">
          {svgNo ? (
            <p className="font-semibold text-[20px] sm:text-[22px] md:text-[26px] lg:text-[32px] text-[#0f6089]">
              {svgNo === 4 ? "iv" : "i".repeat(svgNo)}.
            </p>
          ) : (
            SVG
          )}
        ...
        {para &&
          para.map((para, i) => (
            <p
              key={i}
              className="text-[16px] text-start relative z-10 mr-auto pr-[8%]"
            >
              {para}
            </p>
          ))}
        ...
        {!svgNo && (
          <>
            ...
            <img
              src="/star.png"
              alt="Star"
              className="w-5 h-5 absolute top-5 right-10"
            />
            <img
              src="/star.png"
              alt="Star"
              className="w-5 h-5 absolute top-10 right-5"
            />
          </>
        )}
      </div>
